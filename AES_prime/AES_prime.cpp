...
// We need 44 words in key scheduling where each word is of 32 bit. So i am storing all 44 word in matrix of 44 X 4.
...
void createWords(unsigned char key[4][4]) {
    // Generate words[0], words[1], words[2], words[3] from the given key matrix
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            words[i][j] = key[j][i];
        }
    }
    // Generate words[4] to words[43] using for loop i -> 4 to 43
    for (int i = 4; i < 44; i++) {
        unsigned char keyTemp[4];
        for (int j = 0; j < 4; j++) {
            keyTemp[j] = words[i - 1][j];
        }
        if (i % 4 == 0) {
            rotWord(keyTemp);
            subWord(keyTemp);
            for (int j = 0; j < 4; j++) {
                keyTemp[j] = keyTemp[j] ^ Rcon[i / 4 - 1][j];
            }
        }
        for (int j = 0; j < 4; j++) {
            words[i][j] = words[i - 4][j] ^ keyTemp[j];
        }
    }
}
...
int main(){
    ...
    std::cout << "Enter PlainText in 16 HexaDecimal: ";
    for(int i = 0 ; i<4 ; i++){
        for(int j = 0 ; j<4 ; j++){
            int temp;
            std::cin >> std::hex >> temp; // taking input as hex into an int
            plaintext[j][i] = static_cast<unsigned char>(temp); // casting int to unsigned char
        }
    }
 
    std::cout << "Enter key in 16 HexaDecimal: ";
    for(int i = 0 ; i<4 ; i++){
        for(int j = 0 ; j<4 ; j++){
            int temp;
            std::cin >> std::hex >> temp; // taking input as hex into an int
            key[j][i] = static_cast<unsigned char>(temp); // casting int to unsigned char
        }
    }
    ...
    for(int i = 0 ; i< 4 ; i++){
        for(int j = 0 ; j<4 ; j++){
            std::cout << std::hex << (int)plaintext[j][i] << " "; // out cipher text is stored in plaintext array
        }
    }
    ...
    for(int i = 0 ; i< 4 ; i++){
        for(int j = 0 ; j<4 ; j++){
            std::cout << std::hex << (int)plaintext[j][i] << " "; // out decrypted text is stored in plaintext array
        }
    }
